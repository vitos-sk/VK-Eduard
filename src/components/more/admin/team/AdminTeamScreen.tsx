...
import { DailyNormEditor } from "@/components/more/admin/team/DailyNormEditor";
...
/**
 * Розділ «Команда» адмінки (`/more/admin/team`, тільки boss). Форму
 * заведення (`AddWorkerForm`) і кнопку деактивації (`DeactivateWorkerButton`)
 * переносимо сюди з вкладки «Команда» на `/reports` — та ж бізнес-логіка
 * (`createWorker`/`setWorkerActive`), новий тут лише редактор денної норми
 * (`DailyNormEditor`) і годинники за поточний місяць поруч зі списком.
 */
export function AdminTeamScreen({ profile, workers: initialWorkers, entries }: AdminTeamScreenProps) {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pb-24 lg:px-0 lg:pb-0">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex h-11 flex-1 items-center gap-2 rounded-[12px] border border-border bg-surface-2 px-3 focus-within:border-brand lg:max-w-[320px]">
          ...
          <input
            ...
            aria-label={t.admin.team.searchPlaceholder}
      ...
      {visibleWorkers.length === 0 ? (
        <EmptyState
          title={hasActiveSearch ? t.admin.team.emptySearchTitle : t.admin.team.empty}
          description={hasActiveSearch ? t.admin.team.emptySearchHint : t.admin.team.emptyHint}
        />
      ) : (
