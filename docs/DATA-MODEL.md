...
| `work_entries` | читает и создаёт **только свои**, правит и удаляет свои за последние 7 дней | читает, правит и удаляет все в компании |
| `entry_photos` | по правам родительской записи, **и то же окно 7 дней** | то же, без окна |
...
Окно на `entry_photos` появилось не сразу: изначально фото можно было добавить
или удалить у записи любой давности — несогласованность вскрылась на этапе 4,
когда дошло до кода, который правда загружает фото (миграция
...
Удаление разрешено в том же окне, что и правка (`0005_entries_delete.sql`): рабочий
удаляет свои записи за последние 7 дней, шеф — любые. Раніше видалення було
заборонено всім навмисно («табель — фінансовий документ»), рішення переглянуте
за прямим запитом власника продукту.
