...
`0004_photos_edit_window.sql` — на этапе 4, когда появился код, который правда
загружает фото, и стало видно, что `entry_photos` не наследует окно правки
work_entries (подробности в комментариях миграций и в docs/ROADMAP.md).
Дальнейшие изменения — новой миграцией, а не правкой этого файла и не руками
в дашборде.
...
| `work_entries` | читает и создаёт **только свои**, правит и удаляет свои — без ограничения по давности | читает, правит и удаляет все в компании |
| `entry_photos` | по правам родительской записи | то же |

Окно правки в 7 дней (`0001_init.sql`/`0004_photos_edit_window.sql`/`0005_entries_delete.sql`)
убрано миграцией `0007_remove_entries_edit_window.sql` — по прямому запросу
владельца продукта каждый работник должен всегда мочь редактировать и удалять
**свои** записи, независимо от даты. Чужие записи рабочему по-прежнему недоступны.
...
create policy entries_update on work_entries for update
  using (
    ...
    and (is_boss() or author_id = auth.uid())
  );

create policy entries_delete on work_entries for delete
  using (
    company_id = current_company_id()
    and (is_boss() or author_id = auth.uid())
  );
...
Удаление и правка разрешены в одном и том же объёме (`0005_entries_delete.sql`):
рабочий — свои записи, шеф — любые. Раніше видалення було заборонено всім
навмисно («табель — фінансовий документ»), рішення переглянуте за прямим
запитом власника продукту.
